...
class DirectedGraph : public Graph {//有向简单图类，不含自环，不含重边
    protected:
        std::unordered_map<int, std::unordered_set<Vertex*>> adjList_; // 邻接表，为有向简单图类特有,a[i]存储i的出邻接点集合
        std::unordered_set<Edge*> edges_; // 边集合，为有向简单图类特有
        std::vector<Vertex*> vertices_; // 顶点集合，为有向简单图类特有
        //DirectedGraph* original_graph_ = nullptr; // 原图，为有向简单图类特有，用于子图算法
        const bool is_directed_ = true; // 是否为有向图
    public:
        ~DirectedGraph() override; // 析构函数
        ...
        int degree(int v) const override; // 顶点v的出度
